...
def execute_noise_vs_dd_sweep(TEST_DEGREE):
    print(f"\n--- Mapping 2D Phase Diagram: DD Ratio vs. SPAM Error (Degree {TEST_DEGREE}) ---")
    ...
    spam_errors = np.arange(0.0, 0.026, 0.005) # 0.0% to 2.5% in 0.5% steps
    
    fidelity_matrix = np.zeros((len(spam_errors), len(dd_ratios)))

    for i, err in enumerate(spam_errors):
        print(f"\n--- Testing SPAM Error Rate at {err*100:.2f}% ---")
        
        # Internal operations are perfect; zero out phase drift
        ...
        for j, ratio in enumerate(dd_ratios):
            ...
            for scale in [1, 3, 5]:
                ...
                # Set 1Q and 2Q gate errors to 0.0; inject only SPAM error
                model = get_comprehensive_noise_model(t1_us=np.inf, t2_us=np.inf, pulse_err_1q=0.0, 
                                                     pulse_err_2q=0.0, spam_error=err*scale)
                
                counts = run_local_simulation(circ, noise_model=model, shots=3000)
                # By passing p_error=0.0, we leave the SPAM error unmitigated to track its raw impact
                m3_fid = apply_matrix_inversion(counts, 3000, p_error=0.0)
    ...
    im = plt.imshow(fidelity_matrix * 100, cmap='plasma', aspect='auto', origin='lower',
                    extent=[-12.5, 112.5, spam_errors[0]*100-0.25, spam_errors[-1]*100+0.25], vmin=0, vmax=100)
    
    cbar = plt.colorbar(im)
    cbar.set_label('Mitigated Output Fidelity (%)', rotation=270, labelpad=20)
    
    plt.xticks([0, 25, 50, 75, 100], ['0%', '25%', '50%', '75%', '100%'])
    plt.yticks(spam_errors * 100)
    plt.xlabel('Dynamical Decoupling Protection Ratio (%)')
    plt.ylabel('SPAM Error (%)')
    plt.title(f'Phase Diagram: Degree {TEST_DEGREE} SPAM Tolerance')
    
    for idx_y in range(len(spam_errors)):
        for idx_x in range(len(dd_ratios)):
            val = fidelity_matrix[idx_y, idx_x] * 100
            text_color = "black" if val > 70 else "white"
            plt.text(dd_ratios[idx_x]*100, spam_errors[idx_y]*100, f'{val:.1f}', 
                     ha="center", va="center", color=text_color, fontsize=10)
                     
    output_filename = f'spam_vs_dd_sweep_degree_{TEST_DEGREE}.png'
